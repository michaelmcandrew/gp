...
//        CRM_Utils_System::authenticateScript( true );
    ...
    public function updateGreeting( $greeting )
    {
	    $config = CRM_Core_Config::singleton();
...
SELECT DISTINCT id, $idFldName
 ...
 WHERE contact_type = %1 AND (LENGTH(first_name) OR LENGTH(first_name))
   AND ( {$idFldName} IS NULL OR 
         ( {$idFldName} IS NOT NULL AND {$displayFldName} IS NULL ) )
   ";
				echo __LINE__.' -> '.date('r')."\n";
        ...
        foreach ( $greetingDetails as $contactID => $contactDetails ) {
			...
			$greetingString=trim(preg_replace('/\h+/', ' ', $greetingString));
...
$obj->updateGreeting( 'email_greeting' );
$obj->updateGreeting( 'postal_greeting' );
$obj->updateGreeting( 'addressee' );
