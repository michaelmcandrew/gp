 ...
 *
 * Reports on UKGR's custom "local capitation" field now instead of
 * total_amount.
 */
...
class CRM_Report_Form_Contribute_LocalCapitationSummary extends CRM_Report_Form {
   ...
   protected $_customGroupGroupBy = true;
    ...
    function __construct( ) {
        ...
        $this->_columns['civicrm_value_capitation_4'] = array(
          'fields' => array(
            'custom_12' => array(
              'name'        => 'local_capitation_12',
              'title'       => ts( 'Capitation Statistics' ),
              'type'        => CRM_Utils_Type::T_MONEY,
              'default'     => true,
              'required'    => true,
              'statistics'  => array('sum'   => ts( 'Aggregate Local Capitation' ),
                                     'count' => ts( 'Donations' ),
                                     'avg'   => ts( 'Local Capitation Average' ), ), ), ),
        );
    ...
    function statistics( &$rows ) {
        ...
        if ( ! $this->_having ) {
            $select = "
            SELECT COUNT({$this->_aliases['civicrm_value_capitation_4']}.local_capitation_12 )       as count,
                   SUM({$this->_aliases['civicrm_value_capitation_4']}.local_capitation_12 )         as amount,
                   ROUND(AVG({$this->_aliases['civicrm_value_capitation_4']}.local_capitation_12), 2) as avg
            ";
