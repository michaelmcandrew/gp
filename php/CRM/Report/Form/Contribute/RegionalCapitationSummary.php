...
/*
 * Modified by Wes Morgan for Third Sector Design - Aug. 2010
 *
 * Reports on UKGR's custom "regional capitation" field now instead of
 * total_amount.
 */
...
class CRM_Report_Form_Contribute_RegionalCapitationSummary extends CRM_Report_Form {
   ...
   protected $_customGroupGroupBy = true;
    ...
    function __construct( ) {
        ...
        // add capitation statistics
        $this->_columns['civicrm_value_capitation_4'] = array(
          'fields' => array(
            'custom_12' => array(
              'name'        => 'regional_capitation_11',
              'title'       => ts( 'Capitation Statistics' ),
              'type'        => CRM_Utils_Type::T_MONEY,
              'default'     => true,
              'required'    => true,
              'statistics'  => array('sum'   => ts( 'Aggregate Regional Capitation' ),
                                     'count' => ts( 'Donations' ),
                                     'avg'   => ts( 'Regional Capitation Average' ), ), ), ),
        );
    ...
    static function formRule( $fields, $files, $self ) {  
        ...
        //grouping criteria
        $ignoreFields = array( 'custom_12', 'display_name' );
    ...
    function statistics( &$rows ) {
        ...
        if ( ! $this->_having ) {
            $select = "
            SELECT COUNT({$this->_aliases['civicrm_value_capitation_4']}.regional_capitation_11 )       as count,
                   SUM({$this->_aliases['civicrm_value_capitation_4']}.regional_capitation_11 )         as amount,
                   ROUND(AVG({$this->_aliases['civicrm_value_capitation_4']}.regional_capitation_11), 2) as avg
            ";
