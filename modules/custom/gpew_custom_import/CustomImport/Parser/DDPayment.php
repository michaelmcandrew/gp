		...
		function parseCandidate(){
			...
			if($this->searchForTGP() != 'one'){
				if($this->searchForTGP() == 'none'){
					$this->addReportLine('warning', "Not able to import payment for {$this->getCurrent('tgp')} (no matching TGP number).");
				}
				if($this->searchForTGP() == 'multiple'){
					$this->addReportLine('warning', "Not able to import payment for {$this->getCurrent('tgp')} (multiple matching TGP numbers).");
				}
				return;
			}
